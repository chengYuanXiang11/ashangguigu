...
export default {
   ...
   */
  isPermission(button, judgeList) {
    var show = false
    var buttonList = JSON.parse(sessionStorage.getItem('buttonList')) || []
    if (buttonList.length == 0) show = true
    ...
    if (!judgeList || judgeList.length == 0) return show
    for (let item of judgeList) {
      if (!item) continue
